...
public class Saida {
    ...
    public void relatorio1(Programa programa, Funcoes funcoes) {
        ...
        for (Map.Entry<String, Disciplina> dis : programa.getDisciplinas().entrySet()) {
            ...
            string = "";
            try (BufferedWriter bf = new BufferedWriter(new FileWriter("1-pauta-" + disciplina.getCodigo() + ".csv"))) {// abre/cria
                                                                                                                        // o
                                                                                                                        // arquivo
                                                                                                                        // e
                                                                                                                        // o
                ...
                for (Map.Entry<Integer, Aluno> me : disciplina.getListAluno().entrySet()) {// acessa os alunos presentes
                                                                                           // na disciplina
                    aluno = me.getValue();// associa o aluno a uma variavel
                    notap = notaf = media = peso = div = 0;// zera as variaveis

                    string += aluno.getMatricula() + ";" + aluno.getNome() + ";";// coloca as informações dos alunos a
                                                                                 // uma unica string string

                    if (!disciplina.getListProva().isEmpty()) {
                        for (Map.Entry<String, Prova> p : disciplina.getListProva().entrySet()) {
                            string += String.format("%.2f", p.getValue().getNota(aluno.getMatricula())) + ";";// coloca
                                                                                                              // as
                                                                                                              // provas
                                                                                                              // na
                                                                                                              // string
                    ...
                    if (!disciplina.getListTrabalho().isEmpty()) {
                        for (Map.Entry<String, Trabalho> t : disciplina.getListTrabalho().entrySet()) {
                            string += String.format("%.2f", t.getValue().getNota(aluno.getMatricula())) + ";";// coloca
                                                                                                              // os
                                                                                                              // trabalhos
                                                                                                              // na
                                                                                                              // string
                    ...
                    if (notap < 7) {// verifica se o aluno passou direto
                        if (disciplina.temFinal()) {
                            if (disciplina.getProvaF().getListAluno().containsKey(aluno.getMatricula())) {// se o aluno
                                                                                                          // fez a prova
                                string += String.format("%.2f", disciplina.getProvaF().getNota(aluno.getMatricula()))
                                        + ";";// pega a da prova e coloca na string
                                notaf = (notap / 2) + (disciplina.getProvaF().getNota(aluno.getMatricula()) / 2);// nota
                                                                                                                 // final
                        ...
                        string += "-;" + String.format("%.2f", notap);// caso tenha passado direto a nota da final não
                                                                      // existe
                    }
                    string += "&";// separação de linha
                }
                String[] alunos = string.split("&");// adiciona um token no final de cada aluno para que seja separada
                ...
                for (int i = 0; i < aux; i++) {// adiciona a string dos alunos uma a uma no arquivo, e salta para a
                                               // proxima linha
                ...
                for (int i = 0; i < aux; i++) {// adiciona os alunos de acordo com o nome
                    for (int j = 0; j < aux; j++) {
                        String[] separa = alunos[j].split(";");// separa os alunos para poder comparar o nome com a
                                                               // ordem
    ...
    public void relatorio2(Programa programa) {
        ...
        float media = 0, div = 0;
        String string = "";
        try (BufferedWriter bf = new BufferedWriter(new FileWriter("2-disciplinas.csv"))) {// abre/cria o arquivo e o
                                                                                           // buffer de escrita
            ...
            for (Map.Entry<String, Disciplina> dis : programa.getDisciplinas().entrySet()) {// pega todas as disciplinas
                ...
                for (Map.Entry<Integer, Curso> me2 : programa.getCursos().entrySet()) {// pega todos os cursos
                    ...
                    if (curso.getListDisciplinas().containsKey(disciplina.getCodigo())) {// verifica se a disciplina
                                                                                         // está presente nesse curso
                        for (Map.Entry<Integer, Aluno> me : disciplina.getListAluno().entrySet()) {// pega os alunos da
                                                                                                   // disciplina

                            aluno = me.getValue();// atribui a uma variavel
                            // PROCURANDO O ALUNO UM A UM DENTRO DO CURSO
                            if (curso.getListAlunos().containsKey(aluno.getMatricula())) {// verifica se o aluno faz
                                                                                          // parte do curso
                                ...
                                if (disciplina.getNotaf(aluno.getMatricula()) >= 5.0) {// verifica quem teve uma nota
                                                                                       // final maior que 5
                        ...
                        string += (disciplina.getCodigo() + ";" + disciplina.getNome() + ";" + curso.getNome() + ";"
                                + String.format("%.2f", media) + ";" + String.format("%.1f", passou) + "%" + "&");// escreve
                                                                                                                  // na
                                                                                                                  // string
                        // zera variaveis
                        div = media = 0;
    ...
    public void relatorio3(Programa programa, SimpleDateFormat sdf) {// relatoria de avaliacao
        ...
        try (BufferedWriter bf = new BufferedWriter(new FileWriter("3-avaliacoes.csv"))) {
            ...
            for (Map.Entry<String, Disciplina> me : programa.getDisciplinas().entrySet()) {
                ...
                /*
                 * if(disciplina.temFinal()){
                 * ava=disciplina.getProvaF();
                 * string+=disciplina.getCodigo()+";"+ava.getCodigo()+";"+ava.getNome()+";"+sdf.
                 * format(ava.getData())+";"+String.format("%.2f",ava.mediaAvaliativa())+"&";
                 * }
                 */
            }
            ...
            for (int i = 0; i < aux - 1; i++) {// adiciona a string dos alunos uma a uma no arquivo, e salta para a
                                               // proxima linha
